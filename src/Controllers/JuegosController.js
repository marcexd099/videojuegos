...
const eliminarImagen = async(id) => {
    ...
    if (fs.existsSync('./public' + img)) { // Verifica si el archivo existe antes de eliminarlo.
...
const validar = (nombre,niveles,fecha,img,sevalida) =>{
    ...
    if(niveles === undefined || niveles.trim() === '' || isNaN(niveles)){
        errors.push('El numero de niveles NO debe de estar vacio y debe ser numerico')
    }
    if(fecha === undefined || fecha.trim() === '' || isNaN(Date.parse(fecha))){
        errors.push('La fecha NO debe de estar vacio y debe ser fecha valida')
    }
